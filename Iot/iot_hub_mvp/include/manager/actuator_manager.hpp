...
public:
    ...
    // 공통 초기화/정리
    bool init();          // DHT11 idle 설정, IR 입력/글리치 필터 설정
    ...
    // 백그라운드 환경 폴링
    bool start_env_poll(std::chrono::milliseconds interval, EnvCallback cb);
    void stop_env_poll();

    // IR 파라미터 재설정
    bool set_ir_glitch_us(int glitchUs); // 재적용 위해 init 재호출
    void set_ir_gap_us(int gapUs);       // 다음 캡처부터 적용(객체 내부 gap 유지)
